...
for i, lr in enumerate([0.001, 0.01, 0.1]):
    ...
    cmd = f'sbatch  -J {jobname} -N 1 --ntasks-per-node=1 -A m4731_g -t {hours} -o {out} -e {err} --gres=gpu:1 --wrap="python {script} --nepoch 100 --lr {lr} --batch_size=64 --verbose --maxImgs 10000 --nwork 0 --outdir {odir}" -q {queue} -C gpu'
    print(cmd)
